...
OrientedLineSegment::OrientedLineSegment(void) {
	x1 = x2 = y1 = y2 = 0;
}
...
float OrientedLineSegment::getWeight(const int &x, const int &y) const {
	float w = (float)pow(pow(length(), OrientedLineSegment::P) / (OrientedLineSegment::A + fabs(distance(x, y))), OrientedLineSegment::B);
	if (isinf(w) || isnan(w)) {
		printf("w %f length %f distance %f\n", w, length(), distance(x, y));
	}
	return w;
}
...
void OrientedLineSegmentPairs::getSourcePosition(const int &targetX, const int &targetY, float &sourceX, float &sourceY) const {
	...
	for (int i = 0; i < count; i++) {
		...
		totalDY += dy * weight;
		totalWeight += weight;
		if (isinf(totalWeight) || isnan(totalWeight)) {
			printf("weight %f weight %f\n", weight, weight);
		}
	}
	totalDX /= totalWeight;
	totalDY /= totalWeight;
	...
	if (isnan(totalDX) || isnan(totalDY)) {
		printf("totalWeight %f totalWeight %f\n", totalWeight, totalWeight);
	}
}
